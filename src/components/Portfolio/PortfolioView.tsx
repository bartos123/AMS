import { PortfolioStats, PortfolioViewProps, SearchSuggestion, StockItem } from '../../types/portfolio';
...
export const PortfolioView = ({ 
  ...
  searchInputRef, searchContainerRef, saveAndSet 
    ...
    const emptySlots = watchlist.length > 0 ? (4 - (watchlist.length % 4)) % 4 : 0;
  return (
  <section className="h-full flex flex-col animate-in slide-in-from-right-8 fade-in duration-700 overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row gap-0 group border-x-2 border-t-2 border-black bg-white">
        ...
        <div className="relative flex-1">
          ...
          {suggestions.length > 0 && (
            /* FIX: Odstraněno border-t-0. 
               Ponecháním border-2 získá první výsledek horní linku, 
               která vizuálně oddělí input od našeptávače.
            */
    ...
    {/* GRID AREA - gap-0 zajišťuje, že čáry na sebe navazují */}
    <div className="flex-1 overflow-y-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-0 bg-black border-2 border-black auto-rows-max">
      {watchlist.length === 0 ? (
        <div className="col-span-full py-40 text-center bg-white">
          <h3 className="text-xl font-black text-black/10 uppercase tracking-[0.5em]">
            — Empty Registry —
          </h3>
        </div>
      ) : (
        <>
          {watchlist.map((item : StockItem ) => (
            <div key={item.symbol} className="bg-white min-h-[400px]">
              <StockCard 
                symbol={item.symbol} 
          ...
          {[...Array(emptySlots)].map((_, i) => (
            <div key={`empty-${i}`} className="bg-white border-b border-r border-black flex items-center justify-center">
    ...
    {/* FOOTER */}
    {watchlist.length > 0 && (
      <div className="py-12 flex justify-center bg-white border-t-2 border-black">
        <button 
          onClick={() => {if(confirm('PURGE DATABASE?')) saveAndSet([])}} 
          className="text-[10px] font-black uppercase tracking-[0.4em] text-black/20 hover:text-red-600 transition-colors "
        >
