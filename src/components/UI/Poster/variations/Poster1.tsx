...
export const Poster = () => {
  return (
    <section className="h-screen w-full bg-white text-black flex flex-col overflow-hidden select-none relative animate-in fade-in duration-1000">
      ...
      {/* 1. HORNÍ POLOVINA: TRŽNÍ ŠUM / DATA (Hustý rastr) */}
      <div className="flex-1 w-full bg-black flex justify-between items-stretch px-[10%] pt-20">
        {Array.from({ length: 60 }).map((_, i) => (
          <div 
            ...
            style={{ 
              // Náhodná výška pro evokaci volatility trhu, ale v přísném řádu
      ...
      {/* 2. DOLNÍ POLOVINA: SPRÁVA AKTIV / STRUKTURA (Široké bloky) */}
      <div className="flex-1 w-full bg-white flex justify-between items-stretch px-[10%] pb-20">
        {Array.from({ length: 12 }).map((_, i) => (
          <div 
            ...
            style={{ 
              // Širší rozestupy evokují stabilitu a vybraná aktiva
              height: `${100 - (i % 3) * 10}%`
            }} 
          />
        ))}
      ...
      {/* 3. CENTRÁLNÍ TEXTOVÝ BLOK (Informační jádro) */}
      ...
      {/* 4. OKRAJOVÉ DEKORY (Technické souřadnice) */}
